...
export class CoreUI extends cc.Component {
    ...
    onLoad() {
        let self = this;
        this.lblFiatBalance.formatterFunc = (n: number) => BalanceFormatter.formatCNY(n) + 'USD';
    ...
    update(dt: number) {
...

        {
            ...
            if (this.lastOperDir) {
                ...
                this.lblLastOper.string = this.lastOperDir == 1 ? 'Last Buy' : 'Last Sell';
    ...
    refreshBalance() {
        this.lblFiatBalance.setTargetNumber(Math.max(0, this.fiatBalance));
