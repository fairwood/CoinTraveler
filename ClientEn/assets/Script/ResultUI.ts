...
export default class ResultUI extends cc.Component {
    ...
    onEnable() {
        ...
        this.lblTotalCNY.string = "=$" + BalanceFormatter.formatCNY(btc * MainCtrl.Instance.lastPrice);
        ...
        if (history.length > maxShowCount) {
            history.splice(5, history.length - maxShowCount, `......(${history.length - maxShowCount} items)`);
        }
        for (let i = 0; i < history.length; i++) {
            ...
            if (typeof (tradeInfo) != 'string') {
                ...
                tradeNode.getChildByName('LblDirection').getComponent(cc.Label).string = tradeInfo[2] == 1 ? 'Buy' : 'Sell';
